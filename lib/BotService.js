...
const updateActivity = (activityData, tenant, company, accessToken) => {
  if (!config.Services || !config.Services.botServiceHost)
    return Promise.reject("Required environment variables not provided.('Services')");

  if (!accessToken || !tenant || !company)
    return Promise.reject("BotService::updateActivity - Required method parameters not provided.");

  const SRVConfig = config.Services,
    SRVUrl = `${SRVConfig.botServiceProtocol || 'https'}://${SRVConfig.botServiceHost}/DBF/API/${SRVConfig.botServiceVersion || '1.0.0.0'}/TenantActivitiesService/${tenant}`; // https://<host>/DBF/API/1.0.0.0/TenantActivitiesService/1

  if (activityData) {
    activityData['insertOrUpdate'] = 'update';
  }
